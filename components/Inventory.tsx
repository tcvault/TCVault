...
import { Search, Trash2, Edit3, X, ChevronDown, Filter as FilterIcon, Plus, BookOpen, Layers, ChevronLeft, ChevronRight, Ghost, RotateCcw } from 'lucide-react';
...
const Inventory: React.FC<InventoryProps> = ({ cards, pages, globalSearch = '', onClearSearch, onDelete, onUpdate, onCreatePage, onDeletePage, initialActiveBinderId = 'all', animationClass }) => {
  ...
  const activePageName = useMemo(() => {
    if (activePageId === 'all') return 'Master Archive';
    ...
    <div className={`space-y-12 pb-24 ${animationClass || 'animate-in fade-in duration-300'}`}>
      ...
      <div className="pb-4 border-b border-white/5">
        {activePageId === 'all' ? (
          <div className="flex items-center gap-4 animate-in fade-in duration-300">
            <div className="flex items-center gap-2 px-6 h-11 rounded-full bg-white text-black font-black shadow-lg">
              <Layers size={16} />
              <span className="text-[10px] uppercase tracking-widest">Master Archive</span>
              <span className="text-[10px] text-black/50 ml-2">{cards.length}</span>
            </div>
            <div className="hidden md:block text-[10px] font-semibold text-slate-600 uppercase tracking-widest">
              Total collection reach
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-4 animate-in slide-in-from-left-2 duration-300">
            ...
            <div className="flex items-center gap-2 px-6 h-11 rounded-full bg-white text-black font-black shadow-lg">
              ...
              <span className="text-[10px] text-black/50 ml-2">{filteredCards.length}</span>
            </div>
            <button 
              onClick={() => onDeletePage(activePageId as string)} 
              className="p-3 min-w-[44px] min-h-[44px] flex items-center justify-center text-slate-700 hover:text-rose-500 transition-colors rounded-xl active:scale-90 ml-2"
              title="Delete Binder"
            >
              <Trash2 size={16} />
            </button>
            ...
            <div key={card.id} className="group cursor-pointer space-y-4" onClick={() => setSelectedCard(card)}>
              <div className="aspect-[3/4] rounded-[16px] overflow-hidden border border-white/5 shadow-lg bg-slate-950 relative flex items-center justify-center p-4 img-loading">
                ...
                <button 
                  ...
                  className="absolute top-2 left-2 p-2.5 bg-white text-black rounded-lg shadow-xl z-30 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity active:scale-95"
                  title="Modify Record"
                >
                  <Edit3 size={14} />
                </button>
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-slate-600 font-semibold uppercase tracking-widest truncate">{card.set} {card.setNumber ? `#${card.setNumber}` : ''}</p>
      ...
      {selectedCard && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-black/90 backdrop-blur-xl animate-in fade-in duration-[300ms]" onClick={() => setSelectedCard(null)}>
          <div className="w-full h-full md:h-auto md:max-h-[90vh] md:max-w-4xl glass md:rounded-[24px] overflow-hidden flex flex-col md:flex-row border-white/10 shadow-2xl relative" onClick={e => e.stopPropagation()}>
             <button onClick={() => setSelectedCard(null)} className="absolute top-4 left-4 md:top-8 md:right-8 md:left-auto z-[110] p-3 min-w-[44px] min-h-[44px] flex items-center justify-center glass-subtle rounded-full text-white hover:bg-white/10 transition-colors active:scale-95 shadow-xl">
               <X size={24} />
             </button>
      ...
      {isCreatingPage && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6" onClick={() => setIsCreatingPage(false)}>
          <div className="w-full max-sm glass rounded-[24px] p-8 space-y-6 animate-in zoom-in-95 duration-[150ms]" onClick={e => e.stopPropagation()}>
...
const FilterSelect = ({ value, onChange, options, placeholder }: any) => (
  <div className="relative group">
    <select 
      ...
      className={`w-full bg-white/[0.03] border rounded-xl h-12 px-4 text-[10px] font-black uppercase tracking-widest appearance-none cursor-pointer outline-none transition-all focus:border-blue-500/40 ${value !== 'all' ? 'border-blue-500/40 text-blue-400' : 'border-white/5 text-slate-600 hover:text-slate-400'}`}
    >
