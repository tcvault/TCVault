...
const Dashboard: React.FC<DashboardProps> = ({ stats, recentCards, onNavigate, onEditCard, animationClass }) => {
  ...
  return (
    <div className={`space-y-16 ${animationClass || 'animate-in fade-in duration-300'}`}>
      ...
      {spotlightCard ? (
        <div className="relative glass rounded-[24px] overflow-hidden border-white/10 group cursor-pointer shadow-xl min-h-[400px]" onClick={() => onNavigate(ViewMode.INVENTORY)}>
          <div className={`absolute inset-0 bg-gradient-to-r from-black via-black/50 to-transparent z-10 transition-opacity duration-1000 ease-out ${isSpotlightLoaded ? 'opacity-100' : 'opacity-0'}`}></div>
          ...
          <div className={`relative z-20 p-8 md:p-16 flex flex-col justify-center max-w-lg h-full space-y-4 transition-all duration-700 delay-300 ${isSpotlightLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
            ...
            <div className="flex items-center gap-12 pt-4">
              <div className="space-y-1">
                <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Market estimate</span>
                <p className="text-2xl font-black text-blue-400 tabular">£{spotlightCard.marketValue.toLocaleString()}</p>
              </div>
              <div className="space-y-1">
                <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Grade</span>
          ...
          <div className="absolute right-0 top-0 bottom-0 w-1/2 overflow-hidden hidden md:flex items-center justify-center bg-black p-10">
            <div className="relative w-full h-full flex items-center justify-center">
              {spotlightCard.images && spotlightCard.images[0] ? (
                ...
                <div className="w-56 aspect-[3/4] rounded-2xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-4 text-slate-700 animate-in fade-in zoom-in-95 duration-700">
          ...
          {recentCards.slice(0, 5).map(card => (
            <div key={card.id} className="group cursor-pointer space-y-4" onClick={() => onNavigate(ViewMode.INVENTORY)}>
              <div className="aspect-[3/4] rounded-[16px] overflow-hidden border border-white/5 shadow-lg bg-slate-950 flex items-center justify-center p-4 relative img-loading">
                ...
                {/* Mobile-friendly edit button in top-left */}
                <button 
