...
const CardForm: React.FC<CardFormProps> = ({ onSubmit, onCancel, initialData, pages, onToast }) => {
  ...
  const [formData, setFormData] = useState<Partial<Card>>({
    ...
    serialNumber: '', notes: '', pageId: '', rarityTier: 'Base', isWishlist: false
  });
  ...
  const runScanner = async () => {
    ...
    setError(null);
    try {
      // Simulate progress steps for better feedback
      const steps = ['Extracting player details...', 'Verifying set parallel...', 'Calculating market value...'];
      let currentStep = 0;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-5 space-y-8">
          <div className="glass rounded-[24px] p-6 space-y-8 border-white/5 relative overflow-hidden shadow-lg">
            {isScanning && (
              <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-xl z-[60] flex flex-col p-8 space-y-10 animate-in fade-in duration-300">
                 ...
                 <div className="flex-1 grid grid-cols-1 gap-8 overflow-hidden pointer-events-none opacity-50">
            ...
            <div className="flex items-center justify-between px-2">
              <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Photos</label>
              ...
              {images.map((img, idx) => (
                <div key={idx} className="aspect-[3/4] bg-slate-950 rounded-xl overflow-hidden relative group border border-white/5 shadow-md flex items-center justify-center p-2 img-loading">
                  <img 
                    ...
                    alt="Preview" 
                  />
                  <button onClick={() => removeImage(idx)} className="absolute top-4 right-4 p-3 min-w-[44px] min-h-[44px] flex items-center justify-center bg-rose-600 text-white rounded-xl opacity-0 group-hover:opacity-100 transition-all active:scale-95 z-20"><Trash2 size={16} /></button>
                </div>
              ))}
              {images.length < 4 && (
                <button 
                  ...
                  disabled={isSaving}
                  className="aspect-[3/4] rounded-xl border border-dashed border-white/10 flex flex-col items-center justify-center gap-4 hover:border-white/20 hover:bg-white/[0.02] transition-all group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 active:scale-[0.98]"
                >
            ...
            {images.length > 0 && (
              <button 
                ...
                className={`w-full h-14 rounded-xl font-black text-sm transition-all flex items-center justify-center gap-2 active:scale-[0.97] active:brightness-90 ${hasScanned ? 'btn-secondary text-slate-400' : 'btn-primary'}`}
              >
                {isScanning ? <Loader2 size={20} className="animate-spin" /> : <Sparkles size={16} />}
                <span className="uppercase text-[10px] tracking-widest">{hasScanned ? 'Rescan card' : 'Identify with AI'}</span>
              </button>
            )}
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-7 space-y-8">
          <div className="glass rounded-[24px] p-8 md:p-12 space-y-8 border-white/5 shadow-lg bg-white/[0.01]">
            {error && <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl flex items-center gap-4 text-rose-400 text-sm font-black tracking-tight animate-in shake duration-500"><AlertCircle size={16} />{error}</div>}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <select value={formData.pageId || ''} onChange={e => setFormData({...formData, pageId: e.target.value})} className={`w-full bg-slate-950/40 border rounded-xl h-12 px-4 outline-none font-black text-sm transition-all appearance-none cursor-pointer focus-visible:ring-2 focus-visible:ring-blue-500/20 ${formData.pageId ? 'border-blue-500/40 text-blue-400' : 'border-white/5 text-slate-500 focus:border-white/20'}`}>
                    <option value="" className="bg-slate-900 font-semibold">Select Binder...</option>
                    {pages.map(p => <option key={p.id} value={p.id} className="bg-slate-900 font-semibold">{p.name}</option>)}
                  </select>
                  <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-600" />
                </div>
                {!formData.pageId && <p className="text-[10px] font-black text-slate-700 uppercase tracking-widest ml-1 animate-pulse">Required organization</p>}
              </div>
            </div>

            <div className="pt-8 border-t border-white/5 space-y-6">
              <div className="flex items-center justify-between p-4 rounded-xl glass-subtle border border-white/5">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${formData.isWishlist ? 'bg-amber-500/10 text-amber-500' : 'bg-slate-500/10 text-slate-500'}`}>
                    <Sparkles size={18} />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-white leading-none">Wishlist Item</p>
                    <p className="text-[10px] text-slate-500 font-semibold">Track cards you're hunting for</p>
                  </div>
                </div>
                <button 
                  type="button"
                  onClick={() => setFormData({...formData, isWishlist: !formData.isWishlist})}
                  className={`w-12 h-6 rounded-full transition-all relative ${formData.isWishlist ? 'bg-amber-500' : 'bg-slate-800'}`}
                >
                  <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${formData.isWishlist ? 'left-7' : 'left-1'}`} />
                </button>
              </div>
          ...
          <div className="flex gap-4">
            <button type="button" onClick={onCancel} className="btn-secondary flex-1 h-14 uppercase text-[10px] tracking-widest">Cancel</button>
            <button 
              type="submit" 
              disabled={isSaving}
              className={`btn-primary flex-[2] h-14 uppercase text-[10px] tracking-widest ${isSaving ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {isSaving ? (
                <>
